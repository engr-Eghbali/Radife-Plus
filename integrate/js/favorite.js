...
$(window).on('load', function () {
  ...
  xhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
      ...
      $(division).attr("id","addedmarket");
      $(division).attr("z-index","9999999");
      division.innerHTML=this.responseText;
     document.getElementById("markets").after(division);
     toPersian();
  ...
  xhttp.open("POST", "http://88.99.111.101:8081/favorite", true);
  xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
  xhttp.send("customer="+localStorage.getItem("radife-phone"));
...
function getgoods(shopid,cat){
  
  localStorage.setItem("shop-req",shopid);
  localStorage.setItem("cat-req",cat);
